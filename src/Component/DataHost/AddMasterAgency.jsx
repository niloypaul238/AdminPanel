...
const AddMasterAgency = () => {
    ...
    const submitedSave = () => {
        setCheckInput("")
        if (changAble.agencyID == "" || changAble.agencyName == "" || changAble.referenceId == "" || changAble.nid == "" || changAble.emiil == ""  || changAble.phone == "" || changAble.ipAdress == "" || changAble.location == "" || changAble.whatsNumner == "" || changAble.location == "") {
            setCheckInput("Please Fill Up Input Field !")
        } else {
            if (passwor === "1234") {
                const allObj = [changAble, ...masterAgencyData]
                setMasterAgencyData(allObj);
                setCheckInput("")
                route('/masterAgency')
            }
            else if (passwor == "") {
            ...
            <div className="">
                <div className="bg-white w-full  rounded-xl shadow p-6">
                    <h2 className="text-2xl font-semibold mb-5 flex items-center gap-x-1">
                        Add Master Agency Details  <span className="text-pink-500"><img src="/Group.png" className='h-4' alt="s" /></span>
                    </h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className='flex gap-y-1 flex-col'>
                            ...
                            <input name='referenceId' onChange={onChangeFun} value={changAble.referenceId} className='border border-gray-600/30 rounded text-gray-700 py-1 px-2' placeholder='1234567890' type="number" />
                        </div>
                        <div className='flex gap-y-1 flex-col'>
                            <label htmlFor="" className='text-lg'>IP Address</label>
                            <input name='ipAdress' onChange={onChangeFun} value={changAble.ipAdress} className='border border-gray-600/30 rounded text-gray-700 py-1 px-2' placeholder='1234567890' type="number" />
                        </div>
                        ...
                        <div className='flex gap-y-1 flex-col'>
                            ...
                            <input name='whatsNumner' onChange={onChangeFun} value={changAble.whatsNumner} className='border border-gray-600/30 rounded text-gray-700 py-1 px-2' placeholder='+1 (737)-123-3265' type="number" />
                        </div>
                    ...
                    <div className="flex justify-end mt-4 gap-3">
                        <Link to={'/masterAgency'} className="px-5 text-center cursor-pointer py-1.5 w-35 border rounded-md">Cancel</Link>
...
export default  AddMasterAgency;